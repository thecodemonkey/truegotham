...
class DistrictController(
        ...
        fun updateDistrictImage(
                @Parameter(description = "City name", example = "Dortmund")
                @PathVariable
                city: String,
                @PathVariable name: String,
        ...
        @io.swagger.v3.oas.annotations.Operation(summary = "Update district facts using AI")
        @org.springframework.web.bind.annotation.PutMapping("/api/districts/{city}/{name}/facts")
        fun updateDistrictFacts(@PathVariable city: String, @PathVariable name: String) =
                districtService.updateDistrictFacts(city, name)

        @io.swagger.v3.oas.annotations.Operation(
                summary = "Update all districts with missing facts"
        )
        @GetMapping("/api/districts/batch/update-facts")
        fun updateAllMissingDistrictFacts() = districtService.updateAllMissingDistrictFacts()
}
