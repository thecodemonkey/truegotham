...
@JsonIgnoreProperties(ignoreUnknown = true) data class OverpassResponse(val elements: List<Way>)
...
class GeocodingService(
    ...
    fun geocodeLocationResult(location: IncidentLocationResult): GeocodedAddress {
        ...
        if (coordinates == null) {
            address =
                    addressNormalizer.combineAddress(
        ...
        return GeocodedAddress(coordinates, address)
    }
            ...
            incident.locations
                    ...
                    .forEach { location ->
                        ...
                        if (address.isNotBlank()) {
                            println(
                                    "${i.toString().padEnd(3)} Geocoding address: ${address.padEnd(40)}  - ${location.coordinates}"
                            )
    ...
    fun geocodeIntersections(street1: String, street2: String): Geocoordinates? {
        ...
        val intersectionAddress =
                "${addressNormalizer.normalize(street1)} / ${addressNormalizer.normalize(street2)}, dortmund"
        val intersectionAddress2 =
                "${addressNormalizer.normalize(street2)} / ${addressNormalizer.normalize(street1)}, dortmund"
        ...
        result =
                intersectionService.geocode("$street1 / $street2")?.let {
        ...
        if (result == null) {
            result =
                    intersectionService.geocode("$street2 / $street1")?.let {
                        val coords = Geocoordinates(it.first, it.second)
